        ...
        domain.run(function () {
            ...
            middleware = function expressRedisCache_Middleware (req, res, next) {
                ...
                self.get(name, domain.bind(function (error, cache) {
                    ...
                    if ( cache.length ) {
                        if(cache[0].headers !==undefined && cache[0].headers['cache-control'] !==undefined){
                            res.append('cache-control',cache[0].headers['cache-control']);
                        }
                        let currentTime=new Date().getTime();
                        res.append('content-type',cache[0].type || "text/html");
                        res.append('x-expire',cache[0].expire);
                        res.append('x-touched',cache[0].touched);
                        res.append('x-reloadin',cache[0].expire - Math.floor((currentTime-cache[0].touched)/1000) ) ;

                        if(currentTime- cache[0].touched>cache[0].expire*1000){
                            console.log('ERROR REDIS : '+req.originalUrl);
                            console.log(Math.floor((currentTime-cache[0].touched)/1000,cache[0].expire));
                            self.del(name, domain.intercept(function (name, cache) {}));
                        }
