  ...
  function add (name, body, options, callback) {
    ...
    domain.run(function () {
      ...
      if(entry.expire==0){
        ...
        self.emit('message', require('util').format('NOT SET %s ~%d Kb %d TTL (sec)', redisKey, calculated_size, +entry.expire));

        console.log("REDIS EXPIRE 0")

        return callback(null, name, entry);
      }
      ...
      self.client.hmset(redisKey, entry,
          domain.intercept(function (res) {
            ...
            if ( entry.expire > 0 ) {
              let expireValue=self.client.expire(redisKey, +entry.expire,
              ...
              console.log("REDIS EXPIRE STATUS: "+expireValue)
            }
            else
            {
